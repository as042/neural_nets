...
impl Network {
    ...
    #[inline]
    pub fn train(&mut self, input: &Vec<FT<f64>>, desired_output: &Vec<FT<f64>>) -> Vec<f64> {
        ...
        let d = Self::run_no_self(&x.iter().map(|e| <F<f64, f64> as autodiff::NumCast>::from(*e).unwrap()).collect::<Vec<autodiff::F<f64, f64>>>());
        let grad = grad(Self::run_no_self, x.as_slice());

        println!("{}", d.to_f64().unwrap());
        grad
    }
