...
impl Network {    
    ...
    pub fn run(&mut self, settings: &RunSettings) {
        let input: Vec<F<f64, f64>> = settings.input.iter().map(|f| F::new(*f, 0.0)).collect();
    ...
    fn run_diff(x: &[FT<f64>]) -> FT<f64> {
        ...
        let width = x[1].to_usize().unwrap();
        let height = x[2].to_usize().unwrap();
        ...
        for w in 0..(width - 1) * height.pow(2) {
            net.weights[w].value = x[w + 3].into();
        }
        for b in 0..(width -1) * height {
            net.neurons[b].bias = x[b + 3 + (width - 1) * height.pow(2)].into();
        }

        let mut input = Vec::<f64>::default();
        for i in 0..height {
            input.push(x[i + 3 + (width - 1) * height.pow(2) + (width -1) * height].into());
        }
        let mut desired_output = Vec::<FT<f64>>::default();
        for o in 0..height {
            desired_output.push(x[o + 3 + (width - 1) * height.pow(2) + (width -1) * height + height].into());
        }

        let settings;
        if x[0] > 0.0.into() {
            settings = RunSettings::new(input, match x[0].to_i32() {
                Some(1) => ActivationFunction::Sigmoid,
                Some(2) => ActivationFunction::ReLU,
                _ => panic!("Invalid value"),
            })
        }
        else {
            settings = RunSettings::new_with_print(input, match x[0].to_i32() {
                Some(-1) => ActivationFunction::Sigmoid,
                Some(-2) => ActivationFunction::ReLU,
                _ => panic!("Invalid value"),
            })
